...
serve(async (req) => {
  ...
  // Simple authentication via query parameter to bypass Supabase gateway JWT requirement
  const url = new URL(req.url);
  const token = url.searchParams.get("token");
  const expectedToken = "temp-fix-token-20260209";
  
  if (token !== expectedToken) {
    return new Response(JSON.stringify({ ok: false, error: "Unauthorized. Use ?token=temp-fix-token-20260209" }), {
  ...
  try {
    ...
    return new Response(JSON.stringify({ 
      ok: true, 
      message: "Airtime redeem functions created successfully",
      details: "Created v2 function + wrapper functions + permissions + refreshed schema cache"
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
